...
def controlpanel_library(request):
    ...
    if "update" in request.GET:
        ...
        names = ["Document"]
        final = "Report"
        new = LibraryItemType.objects.filter(name=final)
        if new:
            new = new[0]
            for name in names:
                old = LibraryItemType.objects.get(name=name)
                LibraryItem.objects.filter(type=old).update(type=new)
                old.delete()
            new.name = "Report/document"
            new.save()
        ...
        if new:
            ...
            for name in names:
                ...
                LibraryItem.objects.filter(type=old).update(type=new)
                old.delete()
            new.name = "Conference Output"
            new.save()

        names = ["Magazine Article"]
        ...
        if new:
            ...
            for name in names:
                ...
                LibraryItem.objects.filter(type=old).update(type=new)
                old.delete()
            new.name = "Media Article"
            new.save()

        names = ["Dataset", "Data visualisation"]
        final = "Data portal"
        new = LibraryItemType.objects.filter(name=final)
        if new:
            new = new[0]
            for name in names:
                ...
                LibraryItem.objects.filter(type=old).update(type=new)
                old.delete()
            new.name = "Datasets and visualization"
            new.save()
        ...
        if new:
            ...
            for name in names:
                ...
                LibraryItem.objects.filter(type=old).update(type=new)
                old.delete()
            new.name = "Bill/statute"
            new.save()

        new = LibraryItemType.objects.filter(name="Computer Program")
        if new:
            new = new[0]
            new.delete()

        new = LibraryItemType.objects.filter(name="Teaching Material")
        if not new:
            LibraryItemType.objects.create(name="Teaching Material", group="academic", icon="graduation-cap")
