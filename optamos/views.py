...
def project_settings(request, id):
    ...
    if request.method == "POST":
        ...
        project.goal = request.POST.get("goal")
        project.description = request.POST.get("description")
        project.is_locked = True if request.POST.get("is_locked") else False
        project.save()
        ...
        if len(lines) > remaining:
            ...
            for email in valid:
                if (user := User.objects.filter(email=email).first()):
                    ...
                    user.is_staff = False
                    user.save()
                    people = People.objects.create(name=email, email=user.email, user=user, meta_data={"optamos": True, "auto_created": True, "pending_activation": True, "temp_password": password})
                    OptamosUser.objects.create(project=project, user=user, level="regular")
...
def project(request, id, page="home"):
    ...
    if request.method == "POST":
        if project.is_locked:
            messages.warning(request, "This project is locked and changes can not be made.")
        elif page == "criteria":
